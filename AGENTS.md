...
- `ritm_variables`: Composes submitted answers for one `sc_req_item` through `sc_item_option_mtom`, `sc_item_option`, and `item_option_new`. It masks answers when the variable name or label indicates password, token, secret, credential, API key, or private key. Reference values retain raw sys_ids with an explicit warning. MRVS payloads are marked unsupported and are not decoded.
