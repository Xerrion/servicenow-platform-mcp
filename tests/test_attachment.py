...
def auth_provider(settings: Settings) -> OAuthPKCEProvider:
    """OAuthPKCEProvider for the unified attachment test scope."""
...
class TestUnifiedAttachmentWrite:
    ...
    @pytest.mark.asyncio()
    async def test_upload_oversize_returns_error(self, settings: Settings, auth_provider: OAuthPKCEProvider) -> None:
