"""Tests for the unified ``build_query`` tool.

Restored from v0.9.x ``tests/test_table.py::TestBuildQuery`` as a stateless
tool. The ``query_token`` machinery and its dedicated resolution test were
dropped; ``build_query`` now returns the encoded query string verbatim in
``data.query`` for the caller to forward to the ``query`` tool.
"""
...
class TestBuildQuery:
    ...
    async def test_hours_ago_missing_value_returns_error(
        ...
        """Time operator without value key returns error."""
        tools = _register_and_get_tools(settings, auth_provider)
        raw = await tools["build_query"](
